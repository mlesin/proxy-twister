...
pub async fn forward_to_proxy(
    ...
    proxy
        ...
        .await?;
    trace!("Sent authentication request to proxy");
    let mut response = [0u8; 2];
    proxy.read_exact(&mut response).await?;
    trace!("Received authentication response: {:?}", response);
    ...
    trace!("Sending SOCKS5 request to proxy");
    ...
    trace!("Forwarded request to proxy");
    ...
    .await
    {
        Ok(Ok(_)) => trace!("Received proxy response header: {:?}", response_header),
        Ok(Err(e)) => {
    ...
    match response_header[3] {
        IPV4_TYPE => {
            ...
            match timeout(Duration::from_secs(10), proxy.read_exact(&mut addr)).await {
                Ok(Ok(_)) => trace!("Proxy bound IPv4 address: {:?}", addr),
                Ok(Err(e)) => {
        ...
        IPV6_TYPE => {
            let mut addr = [0u8; 18];
            match timeout(Duration::from_secs(10), proxy.read_exact(&mut addr)).await {
                Ok(Ok(_)) => trace!("Proxy bound IPv6 address: {:?}", addr),
                Ok(Err(e)) => {
            ...
            match timeout(Duration::from_secs(10), proxy.read_exact(&mut len)).await {
                Ok(Ok(_)) => {
                    ...
                    match timeout(Duration::from_secs(10), proxy.read_exact(&mut domain)).await {
                        Ok(Ok(_)) => trace!("Proxy bound domain address: {:?}", domain),
                        Ok(Err(e)) => {
